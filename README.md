...
| `okfy propose <bundle> --as <actor> …` | Agent channel: file a concept change into `proposals/` — never into the live map. The actor is required (`claude-code/1.0`, `human:alice`) and recorded as `generated`; `--evidence <kind>=<ref>` states what the change rests on. Refused, each with its own code and way out: `E_PROPOSAL_ACTOR`, `E_PROPOSAL_EVIDENCE`, `E_PROPOSAL_INJECTION` (agent-written memory is scanned before it is filed), `E_PROPOSAL_DUPLICATE` (a create whose title already names a concept — `--extends <id>`), `E_PROPOSAL_REJECTED` (text the owner rejected, unchanged — `--reopen "<what changed>"`), `E_MEMORY_LINE` (the ledger has an unreadable line, so the rejected-content gate cannot run). Every propose, accept and reject is appended to `meta/memory.jsonl`. |
| `okfy review list\|accept\|reject <bundle> …` | Owner gate: review proposals; accept validates before merging, records a `verified` entry bound to the accepted text's sha256, and refuses `E_PROPOSAL_BASE_MOVED` when the concept changed after the proposal was written against it (`review list` shows `base_moved` first). Accept does not re-release the bundle: `release-check` notes how many proposals were accepted since the last package. A proposal filed before base checks existed is accepted with `W_PROPOSAL_UNBASED` in its log line. |
...
| `okfy package <bundle> [--demote-unretrieved]` | Generate `index.md`, `README.md`, `AGENTS.md`, log, and pre-commit hook; records a fingerprint of the concept set (`meta/package.json`) so later mutations make the package provably stale, and saves a FRESH retrieval index — packaging is where contents are declared final, and leaving the cache behind is how `refine → package → eval run → accept` used to record evidence gathered from a pre-edit index. `AGENTS.md` renders its type table from the plan's `types`/`layout` when declared, so an adapted bundle no longer ships a consumption protocol omitting its own types. `index.md` opens with `okf_version: "0.2"` frontmatter (OKF v0.2 §8), and `AGENTS.md` carries the before-task/after-task memory protocol between `<!-- okfy:memory -->` markers. `--demote-unretrieved` lists concepts no recorded eval run retrieved last, under "Not reached by any recorded eval run" — still listed, still linked, retrieval unchanged, and no resident tokens saved. |
...
(read) and `okfy_propose` (writes only to `proposals/`, review gate intact; `actor`
is required, a refusal comes back as `{error, message, way_out}` rather than a
traceback, and only a response carrying `persisted: true` wrote anything).
