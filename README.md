...
| `okfy glean <bundle> [--budget N]` | Queue a second pass over the corpus files the first pass left silent. Appends pending `glean-NN` segments holding exactly the entries — `lines`/`chars` spans copied verbatim, so the gleaner sees the window the first Worker saw and not the whole file — of the assigned files `validate`'s `coverage.uncited` reports. Deliberately not a new provenance mechanism: a glean pass **is another segment**, so the job artifact, the ledger row and the fail-closed `release-check` completeness rule apply to it unchanged, with no exception carved for it. The uncited list comes from the validator rather than being recomputed, so `glean` and `validate` cannot disagree about what was missed. File granularity, matching what coverage measures: a file split across several spans counts as cited when ANY span yielded a concept. Numbering continues across rounds (`glean-02` after `glean-01`), because the earlier id already owns an artifact and a ledger row. Pair it with `plugin/prompts/glean-worker.md`, whose whole design point is that **an empty answer is a correct answer** — a second pass told to find something will. |
