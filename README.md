...
| `okfy index <bundle>` | Build the BM25 + link index into `.okfy-cache/`. The index carries an envelope (`schema`, `source_fingerprint` over every file that enters it including `meta/*`, `content_fingerprint`) and this verb plus `okfy package` are its ONLY writers. Every read verb verifies the envelope and falls back to a fresh in-memory build when the cache is missing, corrupt, foreign-schema or stale — it used to be trusted unconditionally, so an emptied cache returned zero hits while `release-check` stayed green. The cache is a speed-up, never evidence: the acceptance contract digests a freshly built index, never this file. |
