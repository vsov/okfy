...
| `okfy dissent add\|list\|waive <bundle> …` | Append-only merge adjudication ledger (`meta/dissent.jsonl`): a claim that a merge group hides a real distinction, who held it, its source anchor, and how it resolved. A `split` merged anyway requires `--overruled-because`. `waive --owner` pins the concept's SHA-256 as `waiver_fingerprint`, so editing that concept reopens the row. `release-check` consults this **only** when `meta/purpose.md` declares `acceptance.dissent: required` (`E_REL_DISSENT_UNADJUDICATED`/`_OPEN`/`_STALE`, escape hatch `acceptance.allow_open_dissent`). It verifies adjudication happened, never that it was rigorous. |
