...
A `split` **stays open**. It is an unresolved objection, not a justification, so it needs no reason when recorded; only an owner waiver or an actual split of the concept closes it, and a later `no-schism` row cannot. Every row pins an `adjudication_fingerprint` over the merged concept's bytes and the sorted draft ids, so editing the concept or letting a later run add a draft to the group returns it to `stale`.
